...
GDRIVE_FILE_IDS = {
    ...
    "Master_No_PLMN.parquet": "1Simg9uithTM5sRF5IqeSchhGagd1Yczf", # LINK 5G BARU
...
# ================= SMART DATA LOADING (DIET RAM) =================
@st.cache_data(ttl=timedelta(hours=12), max_entries=1) 
def load_data():
    dfs = {}
    keep_keywords = [
        'date', 'time', 'operator', 'opr', 'cluster', 'project_cluster', 'tower', 'site', 'cell', 'moentity',
    ...
    for filename, file_id in GDRIVE_FILE_IDS.items():
        ...
        try:
            ...
            # Hanya ekstrak kolom yang relevan agar RAM tidak bocor
        ...
        gc.collect() # Bersihkan memori setiap 1 file selesai
...
with st.spinner("Mengunduh & Mempersiapkan Data (Optimasi RAM Aktif)..."):
...
# ================= SETUP TABS (ISOLASI KALKULASI AGAR RAM AMAN) =================
tab1, tab2, tab3, tab4 = st.tabs(["📊 Komparasi MOCN", "🏢 Komparasi Operator", "📈 Trend Harian MOCN", "⏳ Trend Busy Hour"])

# ================= TAB 1: PRE VS POST COMPARISON (MOCN RAT) =================
with tab1:
    ...
    # TOMBOL ON-DEMAND: Cegah lag/crash saat user baru pilih tanggal
    if st.button("🚀 Calculate MOCN Comparison", type="primary"):
        if len(pre_dates) == 2 and len(post_dates) == 2:
            ...
            # LOGIKA BASELINE LOCALHOST (KEMBALI KE AWAL!)
            ...
            st.dataframe(df_comp.style.format({f"Pre ({pre_days} Days)": "{:,.2f}", f"Post ({post_days} Days)": "{:,.2f}", "Delta (%)": "{:,.2f} %"}).map(color_delta, subset=['Delta (%)']))
        else:
            st.warning("Please select a complete date range for both Pre and Post calendars above.")

# ================= TAB 2: PRE VS POST COMPARISON & TREND (OPERATOR LEVEL) =================
with tab2:
    ...
    if not chart_lte.empty or not chart_gsm.empty:
        c_lte_pay = get_col(chart_lte, ['totalpayloadgbkpi', '4g_total payload gb_kpi'])
        ...
        c_lte_op = get_col(chart_lte, ['operator', 'opr'])
        c_gsm_op = get_col(chart_gsm, ['operator', 'opr'])
...
with tab4:
    if not chart_4g_bh.empty and len(date_range) == 2:
        ...
        if valid_bh_agg:
            ...
            with col_bh1:
                ...
                if 'BH Availability (%)' in agg_bh.columns:
                    ...
                    st.plotly_chart(format_x_axis(fig_bh_avail, trend_days), use_container_width=True)
        ...
        if unique_sectors > 30: st.info(f"Terdapat {unique_sectors} Tower_Sectors. Silakan filter agar lebih ringan (< 30 Sectors).")
        elif unique_sectors == 0: st.warning("Data Tower_Sector kosong.")
        elif valid_bh_agg:
            if st.button("🔬 Calculate Sector & Cell Analysis", type="primary"):
                ...
                with col_ts1:
                    ...
                    st.plotly_chart(format_x_axis(fig_ts_ta, trend_days), use_container_width=True)
                ...
                with col_ts2:
                    ...
                    st.plotly_chart(format_x_axis(fig_ts_cqi, trend_days), use_container_width=True)

                # CELL LEVEL DALAM BUTTON YANG SAMA
                st.markdown("---")
                ...
                if unique_cells > 30: st.info(f"Terdapat {unique_cells} cells. Silakan filter agar lebih ringan (< 30 Cells).")
                elif unique_cells == 0: st.warning("Data cell kosong.")
                else:
                    ...
                    with col_c1:
                        ...
                        st.plotly_chart(format_x_axis(fig_cell_dl, trend_days), use_container_width=True)
                        ...
                        st.plotly_chart(format_x_axis(fig_cell_pay, trend_days), use_container_width=True)
                        ...
                        st.plotly_chart(format_x_axis(fig_cell_ta, trend_days), use_container_width=True)
                    ...
                    with col_c2:
                        ...
                        st.plotly_chart(format_x_axis(fig_cell_ul, trend_days), use_container_width=True)
                        ...
                        st.plotly_chart(format_x_axis(fig_cell_prb, trend_days), use_container_width=True)
                        ...
                        st.plotly_chart(format_x_axis(fig_cell_cqi, trend_days), use_container_width=True)
